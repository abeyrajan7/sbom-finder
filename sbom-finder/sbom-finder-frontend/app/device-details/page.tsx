    ...
    <>
      ...
      <section className="device-section">
        ...
        {openSections.info && (
          <>
            ...
            <p>
              <strong>Manufacturer:</strong> {deviceDetails.manufacturer}
            </p>
            <p>
              <strong>Category:</strong> {deviceDetails.category}
            </p>
          </>
        )}
      </section>
      ...
      <section className="device-section" onClick={() => toggleSection("os")}>
        ...
        {openSections.os && (
          <>
            <p>
              <strong>OS:</strong> {deviceDetails.operatingSystem}
            </p>
            <p>
              <strong>OS Version:</strong> {deviceDetails.osVersion}
            </p>
            <p>
              <strong>Kernel Version:</strong> {deviceDetails.kernelVersion}
            </p>
          </>
        )}
      </section>
              ...
              deviceDetails.vulnerabilities.length > 0 ? (
                deviceDetails.vulnerabilities.map((vul: Vulnerability, index: number) => (
                  <div className="vuln-card" key={index}>
                    ...
                    <p>{vul.description}</p>
                    <p>
                      <strong>Severity:</strong> {vul.severityLevel}
                    </p>
                    {vul.sourceUrl && (
                      <a
                        href={vul.sourceUrl}
              ...
              <div className="reference-grid">
                {deviceDetails.externalReferences.map((ref: ExternalReference, index: number) => (
                    <div className="reference-card" key={index}>
                      ...
                      <p>
                        <strong>Type:</strong> {ref.referenceType}
                      </p>
                      {ref.referenceLocator.startsWith("http") ? (
                        ...
                        <p>
                          <strong>Locator:</strong> {ref.referenceLocator}
                        </p>
                      )}
                    </div>
                  )
